...
def Local_num_density(logpsi, params, m, L, N_int, chunk_int_size):
    """
    Creates a function to compute the local particle number density estimate for a single configuration.
    ...
    Returns:
      lcl_density_fn: Function returning the particle number density contribution of a sample.
    """
    logpsi = canonicalize_ansatz(logpsi)
    bounds = (0, L)
    ...
    def lcl_density_fn(x, mask_valid, x_eval):
        ...
        num_density_local = n*jnp.exp(2 * log_Psi_numerator_val - integrals)
        return num_density_local
    return lcl_density_fn

def Num_density(logpsi, params,  m, L, N_int, n_samples, n_chains, warmup, sweep_size, pm, n_max, phys_dim, w, chunk_int_size, chunk_size):
    """
    Creates a function to estimate the system's particle number density profile via Monte Carlo sampling.
    ...
    logpsi = canonicalize_ansatz(logpsi)
    lcl_density_fn = Local_num_density(logpsi, params, m, L, N_int, chunk_int_size)
...
def Num_density_estim(density_type):
    """
    Function to select the particle number density estimation strategy.

    Parameters:
      density_type: 
        - 'full': Evaluates particle number density on a full Cartesian grid (both for 1D and 2D).
        - 'averaged': Evaluates radially averaged particle number density profile (only for 2D cae).

    Returns:
      The corresponding particle number density evaluation function.
    """
    if density_type=="full":
        return Num_density_on_grid
    elif density_type=="averaged":
        return Num_density_averaged
    else:
        raise ValueError("Pick a valid particle number density type: 'full' or 'averaged'")
    
def Num_density_on_grid(logpsi, params, m, L, n_0, N_int, n_samples, n_chains, warmup , sweep_size, pm, 
                              n_max, phys_dim, w, bounds, rng, chunk_int_size, chunk_size, chunk_plt_size, num_points):
    """
    Computes the particle number density profile on a regular Cartesian grid.

    Returns:
      dens_mean: Mean particle number density values at each grid point.
      dens_std: Standard error of the particle number density at each grid point.
      points: The coordinates of the grid points.
    """
    density_fn =  jax.jit(Num_density(logpsi, params,  m, L, N_int, n_samples, n_chains, warmup , sweep_size, pm, n_max, phys_dim, w, chunk_int_size, chunk_size))
    low, high = bounds
...
def Num_density_averaged(logpsi, params, m, L, n_0, N_int, n_samples, n_chains, warmup , sweep_size, pm, 
                              n_max, phys_dim, w, bounds, rng, chunk_int_size, chunk_size, chunk_plt_size, n_angles, n_dists):
    """
    Computes the radially averaged particle number density profile.
    
    Constructs a polar grid centered in the box, evaluates the particle number density, and 
    averages over the angular component to return rho(r).

    Returns:
      dens_mean_avg: Radially averaged particle number density mean.
